...
public class Event {
	...
	public Boolean attendersContainsIdStream(Integer id) {
		Member attender = this.eventAttenders.stream().filter(member -> id == member.getMemberId()).findAny()
				.orElse(null);
		if (attender != null) {
			return true;
		}
	...
	public String toString() {
		return "Event [eventId=" + eventId + ", title=" + title + ", description=" + description + ", location="
				+ location + ", dateTime=" + dateTime + ", memberId=" + memberId + ", eventAttenders=" + eventAttenders
				+ "]";
	}
}