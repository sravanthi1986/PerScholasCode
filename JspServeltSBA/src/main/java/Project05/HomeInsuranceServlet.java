...
@WebServlet({"/HomeInsuranceServlet", "/HomeInsuranceServlet/*"})
public class HomeInsuranceServlet extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
		...
		{
			switch(action) {
				case "/HomeInsuranceServlet":
					showLogin(request, response);
					break;
				case "/HomeInsuranceServlet/register":
					showRegistration(request, response);
					break;
				case "/HomeInusuranceServlet/login":
					showWelcomePage(request, response);
					break;
				case "/HomeInsuranceServlet/submitRegistration":
	...
	private void showLogin(HttpServletRequest request, HttpServletResponse response) 
			throws ServletException, IOException {
		RequestDispatcher rd = request.getRequestDispatcher("/WEB-INF/views/project5/login.jsp");
	...
	private void loginMember(HttpServletRequest request, HttpServletResponse response) 
		...
		User5 m = dao.getUserByfirstName("");
	...
	private void showRegistration(HttpServletRequest request, HttpServletResponse response) 
			throws ServletException, IOException {
		RequestDispatcher rd = request.getRequestDispatcher("/WEB-INF/views/project5/register.jsp");
		rd.forward(request, response);
	}
	private void registerMember(HttpServletRequest request, HttpServletResponse response) 
		...
		User5 user = new User5(fName,lName,email,password,new Date());

		
		User5DAO dao = new User5DAO();
		Integer i = dao.createUser(user);
